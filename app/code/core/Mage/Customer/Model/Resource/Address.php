...
class Mage_Customer_Model_Resource_Address extends Mage_Eav_Model_Entity_Abstract
{
    ...
    protected function _afterSave(Varien_Object $address)
    {
    	...
    	if ($address->getId() && ($address->getIsDefaultPermanent() || $address->getIsDefaultResidence() || $address->getIsDefaultBilling() || $address->getIsDefaultShipping())) {
    		$customer = Mage::getModel('customer/customer')
    		->load($address->getCustomerId());
    
    		if ($address->getIsDefaultPermanent()) {
    			$customer->setDefaultPermanent($address->getId());
    		}
    		if ($address->getIsDefaultResidence()) {
    			$customer->setDefaultResidence($address->getId());
    		}
     ...
     * @return Mage_Customer_Model_Resource_Address
     */
    protected function _afterSave1(Varien_Object $address)
    {
        if ($address->getIsCustomerSaveTransaction()) {
            return $this;
        }
