...
class Mage_Customer_Model_Resource_Customer extends Mage_Eav_Model_Entity_Abstract
{
    ...
    {
    	$defaultPermanentId   = $customer->getData('default_permanent');
    	$defaultResidenceId   = $customer->getData('default_residence');
    	$defaultBillingId   = $customer->getData('default_billing');
    	$defaultShippingId  = $customer->getData('default_shipping');
    	foreach ($customer->getAddresses() as $address) {
    		if ($address->getData('_deleted')) {
    			if ($address->getId() == $defaultPermanentId) {
    				$customer->setData('default_permanent', null);
    			}
    			if ($address->getId() == $defaultResidenceId) {
    				$customer->setData('default_residence', null);
    			}
    			...
    			if (($address->getIsPrimaryPermanent() || $address->getIsDefaultPermanent())
    					&& $address->getId() != $defaultPermanentId
    			) {
    				$customer->setData('default_permanent', $address->getId());
    			}
    			if (($address->getIsPrimaryResidence() || $address->getIsDefaultResidence())
    					&& $address->getId() != $defaultResidenceId
    			) {
    				$customer->setData('default_residence', $address->getId());
    			}
    	...
    	if ($customer->dataHasChangedFor('default_permanent')) {
    		$this->saveAttribute($customer, 'default_permanent');
    	}
    	if ($customer->dataHasChangedFor('default_residence')) {
    		$this->saveAttribute($customer, 'default_residence');
    	}
     ...
     * @return Mage_Customer_Model_Resource_Customer
     */
    protected function _saveAddresses1(Mage_Customer_Model_Customer $customer)
    {
        $defaultBillingId   = $customer->getData('default_billing');
        $defaultShippingId  = $customer->getData('default_shipping');
        foreach ($customer->getAddresses() as $address) {
     ...
     * @return Mage_Customer_Model_Resource_Customer
     */
    public function loadByMobile(Mage_Customer_Model_Customer $customer, $mobile, $testOnly = false){
        $adapter = $this->_getReadAdapter();
        $bind    = array('customer_mobile' => $mobile,
                        'entity_type_id' => 1,
                        'attribute_id' => 139
                    );
        $select  = $adapter->select()
        ->from("customer_entity_varchar", array($this->getEntityIdField()))
        ->where('value = :customer_mobile')
