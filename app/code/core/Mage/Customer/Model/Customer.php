...
class Mage_Customer_Model_Customer extends Mage_Core_Model_Abstract
{
    ...
    /** FOLLOWING FUNCTION CREATED BY MARGSHRI DATE (21 SEP 2018)
     * 
    ...
    public function loadByMobile($customerMobile){
        $this->_getResource()->loadByMobile($this, $customerMobile);
        return $this;
    }
    ...
    /**
     * Get customer default permanent address
     *
     * @return Mage_Customer_Model_Address
     */
    public function getPrimaryPermanentAddress()
    {
    	return $this->getPrimaryAddress('default_permanent');
    }
     ...
     * @return Mage_Customer_Model_Address
     */
    public function getDefaultPermanentAddress()
    {
    	return $this->getPrimaryPermanentAddress();
    }
    
    
    
    /**
     * Get customer default residence address
     *
     * @return Mage_Customer_Model_Address
     */
    public function getPrimaryResidenceAddress()
    {
    	return $this->getPrimaryAddress('default_residence');
    }
     ...
     * @return Mage_Customer_Model_Address
     */
    public function getDefaultResidenceAddress()
    {
    	return $this->getPrimaryResidenceAddress();
    }
    ...
    public function getPrimaryAddressIds()
    {
        ...
        if ($this->getDefaultPermanent()) {
        	$ids[] = $this->getDefaultPermanent();
        }
        if ($this->getDefaultResidence()) {
        	$ids[] = $this->getDefaultResidence();
        }
    ...
    public function getPrimaryAddresses()
    {
        ...
        $primaryPermanent = $this->getPrimaryPermanentAddress();
        if ($primaryPermanent) {
        	$addresses[] = $primaryPermanent;
        	$primaryPermanent->setIsPrimaryPermanent(true);
        }
        
        $primaryResidence = $this->getPrimaryResidenceAddress();
        if ($primaryResidence) {
        	$addresses[] = $primaryResidence;
        	$primaryResidence->setIsPrimaryResidence(true);
        }
    ...
    public function isAddressPrimary(Mage_Customer_Model_Address $address)
    {
        ...
        return ($address->getId() == $this->getDefaultPermanent()) || ($address->getId() == $this->getDefaultResidence()) || ($address->getId() == $this->getDefaultBilling()) || ($address->getId() == $this->getDefaultShipping());
    }
    ...
    public function importFromTextArray(array $row)
    {
        ...
        if (!$this->validateAddress($row, 'permanent')) {
        	$this->printError(Mage::helper('customer')->__('Invalid permanent address for (%s)', $row['email']), $line);
        } else {
        	// Handling permanent address
        	$permanentAddress = $this->getPrimaryPermanentAddress();
        	if (!$permanentAddress  instanceof Mage_Customer_Model_Address) {
        		$permanentAddress = Mage::getModel('customer/address');
        	}
        
        	$regions->addRegionNameFilter($row['permanent_region'])->load();
        	if ($regions) foreach($regions as $region) {
        		$regionId = intval($region->getId());
        	}
        
        	$permanentAddress->setFirstname($row['firstname']);
        	$permanentAddress->setLastname($row['lastname']);
        	$permanentAddress->setCity($row['permanent_city']);
        	$permanentAddress->setRegion($row['permanent_region']);
        	if (isset($regionId)) {
        		$permanentAddress->setRegionId($regionId);
        	}
        	$permanentAddress->setCountryId($row['permanent_country']);
        	$permanentAddress->setPostcode($row['permanent_postcode']);
        	if (isset($row['permanent_street2'])) {
        		$permanentAddress->setStreet(array($row['permanent_street1'], $row['permanent_street2']));
        	} else {
        		$permanentAddress->setStreet(array($row['permanent_street1']));
        	}
        	if (isset($row['permanent_telephone'])) {
        		$permanentAddress->setTelephone($row['permanent_telephone']);
        	}
        
        	if (!$permanentAddress->getId()) {
        		$permanentAddress->setIsDefaultPermanent(true);
        		if ($this->getDefaultPermanent()) {
        			$this->setData('default_permanent', '');
        		}
        		$this->addAddress($permanentAddress);
        	} // End handling permanent address
        }

        
        if (!$this->validateAddress($row, 'residence')) {
        	$this->printError(Mage::helper('customer')->__('Invalid residence address for (%s)', $row['email']), $line);
        } else {
        	// Handling residence address
        	$residenceAddress = $this->getPrimaryResidenceAddress();
        	if (!$residenceAddress  instanceof Mage_Customer_Model_Address) {
        		$residenceAddress = Mage::getModel('customer/address');
        	}
        
        	$regions->addRegionNameFilter($row['residence_region'])->load();
        	if ($regions) foreach($regions as $region) {
        		$regionId = intval($region->getId());
        	}
        
        	$residenceAddress->setFirstname($row['firstname']);
        	$residenceAddress->setLastname($row['lastname']);
        	$residenceAddress->setCity($row['residence_city']);
        	$residenceAddress->setRegion($row['residence_region']);
        	if (isset($regionId)) {
        		$residenceAddress->setRegionId($regionId);
        	}
        	$residenceAddress->setCountryId($row['residence_country']);
        	$residenceAddress->setPostcode($row['residence_postcode']);
        	if (isset($row['residence_street2'])) {
        		$residenceAddress->setStreet(array($row['residence_street1'], $row['residence_street2']));
        	} else {
        		$residenceAddress->setStreet(array($row['residence_street1']));
        	}
        	if (isset($row['residence_telephone'])) {
        		$residenceAddress->setTelephone($row['residence_telephone']);
        	}
        
        	if (!$residenceAddress->getId()) {
        		$residenceAddress->setIsDefaultResidence(true);
        		if ($this->getDefaultResidence()) {
        			$this->setData('default_residence', '');
        		}
        		$this->addAddress($residenceAddress);
        	} // End handling residence address
        }
