...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    public function loginAction(){
        ...
        if($isMobile){
            ...
            $block = $this->getLayout()->getBlock("customer_form_login");
            $block->setTemplate('customer/mobile/form/login.phtml');
    ...
    public function loginPostAction(){
        ...
        $this->_redirectUrl($this->getRequest()->getParam('LastVisitedURL'));
        return;
     ...
     * Login post action
     */
    public function loginPostOLDAction()
    {
        if ($this->_getSession()->isLoggedIn()) {
            $this->_redirect('*/*/');
            return;
    ...
    public function logoutAction()
    {
    	...
    	//$this->locationVO = unserialize(Mage::getSingleton('core/session')->getData('LocationVO'));
    	//$this->clientIP   = Mage::getSingleton('core/session')->getData('ClientIP');
    	
    	
        $this->_getSession()->logout()
            ->setBeforeAuthUrl(Mage::getUrl());
        
        //Mage::getSingleton('core/session')->setData('ClientIP', $this->clientIP);
        //Mage::getSingleton('core/session')->setData('LocationVO', serialize($this->locationVO));
            
        
        //echo Mage::getSingleton('core/session')->getData('ClientIP');
        //exit;    
    ...
    public function createAction(){
        ...
        if($isMobile){
            $this->getLayout()->getBlock('root')->setTemplate('page/empty.phtml');
            $block = $this->getLayout()->getBlock("customer_form_register");
            
            $openContainerData = array("Name"=>"MOBILE");
            $mobileNumer = Mage::getSingleton('core/session')->getRegistrationMobileNumber();
            if($mobileNumer != null){
                $customerModel = Mage::getModel(Margshri_Common_VO_Customer_CustomerVO::$MODELPATH);
                $custMobDataObj = $customerModel->getResource()->getByMobileNumber($mobileNumer);
                if($custMobDataObj !== false){
                    if($custMobDataObj['IsMobileOTPVerified'] == 1){
                        if($custMobDataObj['StatusID'] == Margshri_WebPortal_VO_StatusVO::$INACTIVE){
                            $openContainerData = array("Name"=>"PERSONAL", "MobileNumber"=>$mobileNumer);
                        }
                    }else{
                        // Get Last OTP With This Mobile Number
                        $otpModel = Mage::getModel(Dakiya_VO_OTP_OTP_OTPVO::$MODELPATH);
                        $otpDataObj = $otpModel->getResource()->getLastByMobileNumber($mobileNumer);
                        if($otpDataObj !== false){
                            ...
                            $expireTimeStamp = strtotime($otpVO->getExpiredAt());
                            ...
                            if($diffTimeStamp > 0 && $diffTimeStamp <= $expireSec){
                                $openContainerData = array("Name"=>"OTP", "MobileNumber"=>$mobileNumer, "OTPID"=>$otpVO->getID());
                            }
                        }
                    }
                }
            }
            // $block->setTemplate('customer/mobile/form/register/step1.phtml');
            $block->setTemplate('customer/mobile/form/register.phtml');
            $block->setOpenContainerData($openContainerData);
        }
    ...
    public function createPostAction(){
            ...
            try {
                ...
                if (true === $validationResult) {
                    ...
                    if ($customer->isConfirmationRequired()) {
                        ...
                        // Start Margshri Save Customer (02-09-2018)
                        $customerModel = Mage::getModel(Margshri_Common_VO_Customer_CustomerVO::$MODELPATH);
                        $customerVO = new Margshri_Common_VO_Customer_CustomerVO();
                        
                        $customerVO->setMobileNumber($customerData['mobilenumber']);
                        $customerVO->setFirstName($customerData['firstname']);
                        $customerVO->setLastName($customerData['lastname']);
                        $customerVO->setEmailID($customerData['email']);
                        
                        $adapter = $customerVO->getAdapter();
                        // Get By Mobile Number
                        $custMobDataObj = $customerModel->getResource()->getByMobileNumber($customerVO->getMobileNumber());
                        if($custMobDataObj !== false){
                            
                            //$session->setCustomerFormData($this->getRequest()->getPost())
                            //->addException($this->__('Cannot save the customer.'));
                            //$this->_redirectError(Mage::getUrl('*/*/create', array('_secure' => true)));
                            
                            $customerVO->setID($custMobDataObj['ID']);
                            $customerVO->setCustomerID($customer->getData('entity_id'));
                            $customerVO->setStatusID(Margshri_Common_VO_Customer_CustomerStatusVO::$ACTIVE);
        ...
        // $this->_redirectError(Mage::getUrl('*/*/create', array('_secure' => true)));
        $this->_redirectError(Mage::getUrl('common/Frontend_Customer_Registration/step2', array('_secure' => true)));
    }
    ...
    public function forgotPasswordAction(){
        ...
        if($isMobile){
            $this->getLayout()->getBlock('root')->setTemplate('page/empty.phtml');
            $block = $this->getLayout()->getBlock("forgotPassword");
            $block->setTemplate('customer/mobile/form/forgotpassword.phtml');
        }
     ...
     */
    public function forgotPasswordPostNewAction(){
        
        $mobileNumber = (string) $this->getRequest()->getPost('MobileNumber');
        if ($mobileNumber) {
            
            if(!preg_match('/^[0-9]{10}+$/', $mobileNumber)){
                $this->_getSession()->addError($this->__('Invalid mobile number.'));
            ...
            ->loadByMobile($mobileNumber);
            
            if ($customer->getId()) {
                try {
                    $newResetPasswordLinkToken = Mage::helper('customer')->generateResetPasswordLinkToken();
                    $customer->changeResetPasswordLinkToken($newResetPasswordLinkToken);
                    $linkURL = Mage::getUrl().'customer/account/resetpassword/?id='.$customer->getId().'&token='.$newResetPasswordLinkToken;
                    ...
                    $sendSMSVO->setSMSConfigID(1);
                    $sendSMSVO->setSMSContent("Your aapnicity password reset link is : " . $linkURL);
                    $sendSMSVO->setReceiverMobileNO($mobileNumber);
                    ...
                    if($responseVO->getErrorMessage() != null){
                        // Mage::throwException("Could not sent sms please try again after some time.");
            ...
            $this->_getSession()
            ->addSuccess(Mage::helper('customer')->__('If there is an account associated with %s you will receive a sms with a link to reset your password.', Mage::helper('customer')->htmlEscape($mobileNumber)));
            $this->_redirect('*/*/');
            return;
...
//             $customer = Mage::getModel('customer/customer')->getCollection()->addAttributeToSelect('*')->addAttributeToFilter('mobilenumber', $mobileNumber)->getFirstItem();
//             if ($customer->getData("entity_id")) {
//                 try {
//                     $passwordHash = $customer->getData("password_hash");
//                     $password = str_replace("\x0", '', trim($this->_getCrypt()->decrypt(base64_decode((string)$passwordHash))));
//                     $sendSMSVO = new Dakiya_VO_SMS_SendSMS_SendSMSVO();
//                     // $sendSMSVO->setSMSConfigID(Dakiya_VO_Master_SMS_SMSConfigVO::$CUSTOMER_REGISTRATION);
//                     $sendSMSVO->setSMSConfigID(1);
//                     $sendSMSVO->setSMSContent("Your aapnicity account password is : " . $password);
//                     $sendSMSVO->setReceiverMobileNO($mobileNumber);
//                     $sendSMSModel = Mage::getModel(Dakiya_VO_SMS_SendSMS_SendSMSVO::$MODELPATH);
...
//             }
            $this->_getSession()
            ->addSuccess(Mage::helper('customer')->__('If there is an account associated with %s you will receive a sms with your password.', Mage::helper('customer')->htmlEscape($mobileNumber)));
            $this->_redirect('*/*/');
            return;
        } else {
            $this->_getSession()->addError($this->__('Please enter your mobile number.'));
            $this->_redirect('*/*/forgotpassword');
            return;
        }
    }
    ...
    public function resetPasswordAction(){
        ...
        try {
            ...
            if($isMobile){
                ...
                $block->setTemplate('customer/mobile/form/resetforgottenpassword.phtml');
            }
    ...
    { 
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if($_FILES['Image']['tmp_name'] != null && $_FILES['Image']['tmp_name'] != ''){
            	...
            	move_uploaded_file($_FILES['Image']["tmp_name"], Mage::helper("margshri/Utility")->getServerPath() . '/media/customer/profile_pic/' . $customer["entity_id"].'.'.$ext);
            }
            
            $customerVO = new Margshri_Common_VO_Customer_CustomerVO();
            
            $customerModel = Mage::getModel('common/Customer_Customer');
            $customerDataObj = $customerModel->getResource()->getByCustomerID($customer['entity_id']);
            if($customerDataObj !== false){
                ...
                $customerVO->setID(0);
                $customerVO->setCustomerID($customer['entity_id']);
                
            }
            
            $customerVO->setStatusID(Margshri_Common_VO_Customer_CustomerStatusVO::$ACTIVE);
            $customerVO->setFirstName($customer['firstname']);
            $customerVO->setLastName($customer['lastname']);
            //$customerVO->setCustomerImage($customer['customerimage']);
            $customerVO->setMobileNumber($customer['mobilenumber']);
            $customerVO->setEmailID($customer['email']);
            
            if($this->getRequest()->getParam('IsShowProfile')){
                $customerVO->setIsShowProfile(1);
            }else{
                $customerVO->setIsShowProfile(0);
            }
            
            $customerModel->getResource()->saveFrontDB($customerVO);
    ...
    /** MARGSHRI (28-01-2016)
     *  add or update customer account personal information
     */
    public function personalAction()
    {
    	...
    	$block = $this->getLayout()->getBlock('customer_personal');
    	if ($block) {
    		$block->setRefererUrl($this->_getRefererUrl());
    	}
    	$data = $this->_getSession()->getCustomerFormData(true);
    ...
    /** MARGSHRI (04-02-2016)
     *  add or update customer account office detail
     */
    public function officeAction()
    {
    	...
    	$block = $this->getLayout()->getBlock('customer_office');
    	if ($block) {
    		$block->setRefererUrl($this->_getRefererUrl());
    	}
    	$data = $this->_getSession()->getCustomerFormData(true);
    ...
    /** MARGSHRI (04-02-2016)
     *  add or update customer account other information
     */
    public function otherAction()
    {
    	...
    	$block = $this->getLayout()->getBlock('customer_other');
    	if ($block) {
    		$block->setRefererUrl($this->_getRefererUrl());
    	}
    	$data = $this->_getSession()->getCustomerFormData(true);
    ...
    public function addressAction()
    {
    	...
    	//$block = $this->getLayout()->createBlock('customer_other');
    	//$block->setTemplate('webportal/center/content/type7/event/event/detail.phtml');
    	//$this->getLayout()->getBlock('content')->append($block);
    	//$this->renderLayout();
    	
    	$block = $this->getLayout()->getBlock('customer_address');
    	if ($block) {
    		$block->setRefererUrl($this->_getRefererUrl());
    	}
    ...
    /** MARGSHRI (14-10-2016)
     *  show customer bid Points Detail 
     */
    public function bidPointDetailAction(){
    	$this->loadLayout();
    	$block = $this->getLayout()->getBlock('CustomerBidPointDetail');
    	if ($block) {
    		$block->setRefererUrl($this->_getRefererUrl());
    	}
    	$this->renderLayout();
