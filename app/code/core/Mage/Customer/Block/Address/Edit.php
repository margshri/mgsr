...
class Mage_Customer_Block_Address_Edit extends Mage_Directory_Block_Data
{
    ...
    // ADD BY
    public function canSetAsDefaultPermanent()
    {
    	if (!$this->getAddress()->getId()) {
    		return $this->getCustomerAddressCount();
    	}
    	return !$this->isDefaultPermanent();
    }
    
    public function canSetAsDefaultResidence()
    {
    	if (!$this->getAddress()->getId()) {
    		return $this->getCustomerAddressCount();
    	}
    	return !$this->isDefaultResidence();
    }
    ...
    public function isDefaultPermanent()
    {
    	$defaultPermanent = Mage::getSingleton('customer/session')->getCustomer()->getDefaultPermanent();
    	return $this->getAddress()->getId() && $this->getAddress()->getId() == $defaultPermanent;
    }
    
    public function isDefaultResidence()
    {
    	$defaultResidence = Mage::getSingleton('customer/session')->getCustomer()->getDefaultResidence();
    	return $this->getAddress()->getId() && $this->getAddress()->getId() == $defaultResidence;
    }
