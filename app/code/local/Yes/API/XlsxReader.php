...
//			$bE = ($aP['GPF'] && 0x0001) ? TRUE : FALSE;
	...
	function _parse() {
		...
		foreach ($relations->Relationship as $rel) {
			if ($rel["Type"] == Yes_API_XlsxReader::SCHEMA_OFFICEDOCUMENT) {
				...
				$workbookRelations->registerXPathNamespace("rel", Yes_API_XlsxReader::SCHEMA_RELATIONSHIP);
				
				// Read shared strings
				$sharedStringsPath = $workbookRelations->xpath("rel:Relationship[@Type='" . Yes_API_XlsxReader::SCHEMA_SHAREDSTRINGS . "']");
				...
				foreach ($workbookRelations->Relationship as $workbookRelation) {
					if ($workbookRelation["Type"] == Yes_API_XlsxReader::SCHEMA_WORKSHEETRELATION) {
