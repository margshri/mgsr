...
class Margshri_WebPortal_Frontend_Right_BidPointController extends Mage_Core_Controller_Front_Action {
	...
	public function showBidOuterTermsAction(){
		...
		if($isMobile){
		    ...
		    $block->setTemplate('webportal/right/mobile/bidouterterms.phtml');
	...
	public function showBidHowToPlayAction(){
	    ...
	    if($isMobile){
	        $this->getLayout()->getBlock('root')->setTemplate('page/empty.phtml');
	        $block->setTemplate('webportal/right/mobile/bidhowtoplay.phtml');
	        $this->getLayout()->getBlock('content')->append($block);
	    }else{
	...
	public function showBidListAction(){
	    ...
	    if($isMobile){
    		$this->getLayout()->getBlock('root')->setTemplate('page/empty.phtml');
    		$block->setTemplate('webportal/right/mobile/bidlist.phtml');
    		$this->getLayout()->getBlock('content')->append($block);
	    }else{
	...
	public function showBidDetailAction(){
	    ...
	    if($bidVO->getStatusID() == Margshri_WebPortal_VO_Master_Right_BidStatusVO::$TO_BE_OPEN ||
	        $bidVO->getStatusID() == Margshri_WebPortal_VO_Master_Right_BidStatusVO::$RUNNING){
	        
	            $block = $this->getLayout()->createBlock('webportal/Frontend_Right_BidDetail');
	            if($isMobile){
	                $this->getLayout()->getBlock('root')->setTemplate('page/empty.phtml');
	                $block->setTemplate('webportal/right/mobile/biddetail.phtml');
	            }else{
	...
	public function showBidTermsAction(){
		...
		if($isMobile){
		    $this->getLayout()->getBlock('root')->setTemplate('page/empty.phtml');
		    $block->setTemplate('webportal/right/mobile/bidterms.phtml')->setBidID($bidID);
		    $this->getLayout()->getBlock('content')->append($block);
		}else{
	...
	public function showPlayBidAction(){
		...
		if($bidVO->getStatusID() == Margshri_WebPortal_VO_Master_Right_BidStatusVO::$TO_BE_OPEN ||
		    $bidVO->getStatusID() == Margshri_WebPortal_VO_Master_Right_BidStatusVO::$RUNNING){
		        ...
		        if($isMobile){
		            $this->getLayout()->getBlock('root')->setTemplate('page/empty.phtml');
		            $block->setTemplate('webportal/right/mobile/playbid.phtml');
		        }else{
	...
	public function refreshBidAction(){
		try {
			...
			*/
				...
				if($bidTranVO->getID() != null && $bidTranVO->getIsWin() != 1){
					...
					if($lastBidTranDateTimeSec > $currentDateTimeSec){
						...
						if($bidVO->getStatusID() == Margshri_WebPortal_VO_Master_Right_BidStatusVO::$RUNNING){
							...
							$responseData["IsWinner"] = "YES";
							$winnerID = $bidTranVO->getCustomerID();
			...
			if($responseData["IsWinner"] == "YES" && $winnerID != ""){
			    $customer = Mage::getModel('customer/customer')->load($winnerID);
			    $customerimage = $customer->getcustomerimage();
			    $firstName = $customer->getfirstname();
			    $lastName = $customer->getlastname();
			    if($firstName != null && $firstName != '' ){ $firstName = ucfirst(strtolower($firstName));}
			    if($lastName != null && $lastName != '' ){ $lastName = ucfirst(strtolower($lastName));}
			    
			    $winnerPopupHTML = "";
			    $winnerPopupHTML .= "<div style='width: 100%; max-width: 530px; margin: 20px; overflow: hidden; box-shadow: 0 60px 80px 0 rgba(0,0,0,.4); border-top-left-radius: 10px; border-top-right-radius: 10px; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px;'>";
			    $winnerPopupHTML .= "<div style='padding: 30px 30px 20px; background: rgba(251,251,251,1);'>";
			    $winnerPopupHTML .= "<img style='display: block; margin: 0 auto 10px;' src='".Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . 'web_portal/frontend/bid/trophy.png'."' alt='Trophy' width='280px' height='130px' />";
			    
			    $winnerPopupHTML .= "<div style='margin-bottom: 5px; font-size: 25.5px; letter-spacing: 2px; text-align: center;'><img style='display: block; margin: 0 auto 10px;' src='".Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . $customerimage ."' alt='Trophy' width='80px' height='80px' /></div>";
			    $winnerPopupHTML .= "<div style='margin-bottom: 5px; font-size: 25.5px; letter-spacing: 2px; text-align: center;'>".$firstName . " " . $lastName ."</div>";
			    
			    if($this->customerID == $winnerID){
			        $winnerPopupHTML .= "<div style='margin-bottom: 5px; font-size: 25.5px; letter-spacing: 2px; text-align: center;'>Congratulations</div>";
			        $winnerPopupHTML .= "<div style='color: #999; margin: 0 auto; max-width: 350px; font-size: 19px; line-height: 1.3; letter-spacing: 1.25px; text-align: center;'>You are the winner</div>";
			    }else{
			        $winnerPopupHTML .= "<div style='margin-bottom: 5px; font-size: 25.5px; letter-spacing: 2px; text-align: center;'>Winner</div>";
			    }
			    
			    $winnerPopupHTML .= "</div>";
			    $winnerPopupHTML .= "<div style='display: flex; flex-wrap: wrap; padding: 10px 50px; border-bottom-left-radius: 10px !important; border-bottom-right-radius: 10px !important; background: #fff;'>";
			    $winnerPopupHTML .= "<button style='margin: 5px auto; flex: 0 0 100%; padding: 10px 10px; border: 0; border-radius: 25px; color: #fff; font-size: 18px; letter-spacing: 1px; transition: all .2s ease;' onclick='bidTranFormJS.closeWinnerPopUp()'>Close</button>";
			    $winnerPopupHTML .= "</div>";
			    $winnerPopupHTML .= "</div>";
			    $responseData["WinnerPopupHTML"] = $winnerPopupHTML;
			}
	...
	public function getLastNBidTranHTML($bidID, $methodCallingAction){
		...
		if(sizeof($bidTranDataObjs) > 0){
			...
			foreach($bidTranDataObjs as $bidTranDataObj){
				...
				$newBidTranDataObj['CustomerImage'] = $customerimage;
		...
		if(sizeof($newBidTranDataObjs) > 0){
			...
			*/
		    ...
		    $bidTranHTML .= '<div class="card-deck">';
		    $bidTranHTML .= '<div class="card">';
		    $bidTranHTML .= '<div class="card-body" style="padding:0px !important;">';
			...
			foreach($newBidTranDataObjs as $bidTranDataObj){
			    ...
			    $bidTranHTML .= '<div class="media border p-3" style="padding:.5rem !important;">';
			    
			    if($bidTranDataObj['CustomerImage'] != null && $bidTranDataObj['CustomerImage'] != "" ){
			         $bidTranHTML .= '<img src="'. Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . $bidTranDataObj['CustomerImage'].'" alt="John Doe" class="mr-3 mt-3 rounded-circle text-left" style="width:60px; margin-top: 0px !important;">';
			    }else{
			         $bidTranHTML .= '<div class="mr-3 mt-3 rounded-circle text-left" style="width:30px; margin-top: 0px !important;"></div>';
			    }
			    
			    $bidTranHTML .= '<div class="media-body text-center">';
			    $bidTranHTML .= '<h6>'.$bidTranDataObj['CustomerName'].'</h6>';
			    $bidTranHTML .= '<p><small><i>'.$bidTranDataObj['CityName'].'</i></small></p>';
			    $bidTranHTML .= '</div>';
			    
			    $bidTranHTML .= '<div class="text-right" style="width:60px; margin-top: 0px !important; "><h6>'.$bidTranDataObj['BidValue'].'</h6></div>';
	...
	public function showBidPromotionAction(){
	    $this->_initAction();
	    $isMobile = Zend_Http_UserAgent_Mobile::match(Mage::helper('core/http')->getHttpUserAgent(),$_SERVER);	    
	    $block = $this->getLayout()->createBlock('webportal/Frontend_Right_BidPromotion');
	    if($isMobile){
	        $this->getLayout()->getBlock('root')->setTemplate('page/empty.phtml');
	        $block->setTemplate('webportal/right/mobile/bidmenu.phtml');
	        $this->getLayout()->getBlock('content')->append($block);
	    }else{
	        $block->setTemplate('webportal/right/bidpromotion.phtml');
	        $this->getLayout()->getBlock('content')->append($block);
	    }
	...
	public function showPlayerPointsAction(){
	    $this->_initAction();
	    $isMobile = Zend_Http_UserAgent_Mobile::match(Mage::helper('core/http')->getHttpUserAgent(),$_SERVER);
	    $block = $this->getLayout()->createBlock('webportal/Frontend_Right_BidPointDetail');
	    if($isMobile){
	        $this->getLayout()->getBlock('root')->setTemplate('page/empty.phtml');
	        $block->setTemplate('webportal/right/mobile/bidpointdetail.phtml');
	        $this->getLayout()->getBlock('content')->append($block);
	    }else{
	        $block->setTemplate('webportal/right/BidPointDetail.phtml');
	        $this->getLayout()->getBlock('content')->append($block);
	    }
