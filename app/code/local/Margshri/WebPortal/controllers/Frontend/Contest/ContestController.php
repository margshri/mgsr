<?php 
class Margshri_WebPortal_Frontend_Contest_ContestController extends Mage_Core_Controller_Front_Action {
	...
	public function showBidDetailAction(){
		...
		$block->setTemplate('webportal/right/biddetail.phtml')->setBidID($bidID);
		$this->getLayout()->getBlock('content')->append($block);
		$this->renderLayout();
	}
	...
	public function showPlayBidAction(){
		...
		$block->setTemplate('webportal/right/playbid.phtml')->setBidID($bidID);
		$this->getLayout()->getBlock('content')->append($block);
		$this->renderLayout();
	}
	...
	public function playBidAction(){
		try {
			...
			/*
			if($bidTranVO->getBidValue() < $BID_PLAY_WITH_MIN_POINT || $bidTranVO->getBidValue() > $BID_PLAY_WITH_MAX_POINT){
				$responseVO->setErrorKey("InvalidBidValue");
				Mage::throwException("You Can Add Only " . $BID_PLAY_WITH_MIN_POINT . "-" . $BID_PLAY_WITH_MAX_POINT . " In Last Bid Value.");
			}
			*/
			...
			*/
			...
			$clpPointsTranVO->setEntityID(Margshri_WebPortal_VO_Master_Right_EntityVO::$PLAY_BIDDING);
			$clpPointsTranVO->setEntityTransactionID($bidVO->getID());
			$clpPointsTranVO->setPoints($bidValueDifference);
			$clpPointsTranVO->setCreatedAt($serverDate);
			$response = $clpPointsTranModel->getResource()->saveDB($clpPointsTranVO);
			if($response['status'] == "ERROR"){
				Mage::throwException("Error In Redeem Insert Into CLP Points Tran Table.");
			...
			$bidTranHTML = $this->getLastNBidTranHTML($bidTranVO->getBidID());
	...
	public function completeBidAction(){
		try {
			...
			$errorMsg = array(); $errorFlag = false;
			$responseVO = new Margshri_WebPortal_VO_Right_BidTranVO();
			$adapter = new Margshri_WebPortal_VO_Right_BidTranVO();
			$serverDate = date("Y-m-d H:i:s", Mage::getModel('core/date')->timestamp(time()));
			...
			// SET BID STATUS COMPLETED
			if($bidVO->getStatusID() == Margshri_WebPortal_VO_Master_Right_BidStatusVO::$RUNNING){
				
				$bidStartDateTimeStr = $bidVO->getBiddingDate() . " " . $bidVO->getBiddingTime();
				$compareTimeStr = date('H:i:s',strtotime('+'. $BID_COMPLETE_TIME_IN_SEC . ' seconds',strtotime($bidStartDateTimeStr)));
				
				/*
				$responseVO->setErrorKey("NotCompleted");
				$responseVO->setErrorMessage("bid start time => " . $bidStartDateTimeStr . " compate time " . $compareTimeStr);
				$this->getResponse()->setBody( Mage::helper('webportal/Data')->jsonEncode($responseVO->getBaseDataArray()));
				return;
				*/
				
				if(strtotime($compareTimeStr) < strtotime($currentTime)){
					
					// GET LAST BID TRAN FROM BID TRAN TABLE
					$bidTranModel = Mage::getModel('webportal/Right_BidTran');
					$bidTranDataObj = $bidTranModel->getResource()->getLastByBidID($bidTranVO->getBidID());
					
					if($bidTranDataObj === false){
						Mage::throwException("Bid Tran Not Found.");
					}
					...
					// GET LAST BID TIME
					$lastBidDateTime = date("Y-m-d H:i:s",strtotime($bidTranVO->getCreatedAt()));
					$currentDateTime = date("Y-m-d H:i:s", Mage::getModel('core/date')->timestamp(time()));
					
					$differenceTimeStamp = strtotime($currentDateTime) - strtotime($lastBidDateTime);
					$differenceTime = date("H:i:s",$differenceTimeStamp);
					$differenceTimeArray =  explode(":",$differenceTime); 
					$differenceTimeInSec = ($differenceTimeArray[0] * 3600) + ($differenceTimeArray[1] * 60) + $differenceTimeArray[2];   
					
					if($differenceTimeInSec > $BID_GRACE_PERIOD_TIME_IN_SEC){
						...
						//AFTER COMPLETE THE BID EXPECT WINNER RETURN BID POINT TO ALL USER
						$bidTranDataObjs = $bidTranModel->getResource()->getBidTranCustomerWise($bidTranVO->getBidID());
						foreach($bidTranDataObjs as $bidTranDataObj){
							...
							$clpPointsVO = Margshri_WebPortal_VO_Right_CLPPointsVO();
			...
			if($isBidCompleted == true){
				$responseVO->setSuccessMessage("Bid Completed Successfully.");
				$bidTranHTML = $this->getLastNBidTranHTML($bidTranVO->getBidID());
				$responseVO->setResponseData($bidTranHTML);
			}else{
				$responseVO->setErrorKey("NotCompleted");
				Mage::throwException("Bid Not Completed Yet.");
			}	
	...
	public function getLastNBidTranHTML($bidID){
		...
		if(sizeof($bidTranDataObjs) > 0){
			...
			foreach($bidTranDataObjs as $bidTranDataObj){
				...
				$newBidTranDataObj['CustomerName'] = $customer->getfirstname() . " " . $customer->getlastname();
		...
		if(sizeof($newBidTranDataObjs) > 0){
			...
			foreach($newBidTranDataObjs as $bidTranDataObj){
				...
				if($bidTranDataObj["IsWinner"] == "YES"){
					if($counter == 1){
						$isWinner = true;
						$bidTranHTML .= '<td style="border-bottom:none;">';
						$bidTranHTML .= '<span class="record">Winner</span>';
						$bidTranHTML .= '</td>';
					}	
				}	
