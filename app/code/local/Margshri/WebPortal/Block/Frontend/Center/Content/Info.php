<?php
class Margshri_WebPortal_Block_Frontend_Center_Content_Info extends Mage_Core_Block_Template{

	protected $tableCode;
	
	public function __construct(){
		parent::__construct();
		$this->setTemplate('webportal/center/content/entropy.phtml');
		...
		$collection->getSelect()->reset()->from(array("main_table"=>$collection->getMainTable()), array("ID"=>"main_table.ID", "Value"=>"main_table.Value" , "Address"=>"main_table.Address" , "PinCode"=>"main_table.PinCode", "LandLineNumber"=>"main_table.LandLineNumber" , "MobileNumber1"=>"main_table.MobileNumber1" , "MobileNumber2"=>"main_table.MobileNumber2" ,  "DynamicColumn1Value"=>"main_table.Category1Value", "DynamicColumn2Value"=>"main_table.Category2Value", "DynamicColumn3Value"=>"main_table.Category3Value", "IsPaid"=>"main_table.IsPaid", "SubPageTableCode"=>"main_table.SubPageTableCode", "WebsiteLink"=>"main_table.WebsiteLink", "ContactPersonName"=>"main_table.ContactPersonName", "BusinessDay"=>"main_table.BusinessDay", "BusinessHours"=>"main_table.BusinessHours", "SortName"=>"main_table.SortName", "PaymentMethod"=>"main_table.PaymentMethod", "Area"=>"main_table.Area", "main_table.edit"=> new Zend_Db_Expr("'Edit'") ));
		...
		$collection->getSelect()->where('main_table.TableCodes LIKE ? ', '%' . $this->tableCode . '%');
		...
		$universityTableCodes = array('apctwebdeemeduniversity', 'apctwebugcapproveduniversity', 'apctwebprivateuniversities');
		if(!in_array($this->tableCode, $universityTableCodes)){
		...
		$collection->getSelect()->Order('main_table.PriorityID DESC');
