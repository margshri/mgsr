<?php
class Margshri_WebPortal_Block_Frontend_Center_Content_Type10_Hospital_Hospital_Info extends Mage_Core_Block_Template{
	...
	public function __construct(){
		...
		$hospitalTypeModel = mage::getModel("webportal/Master_Center_Content_Type10_Hospital_HospitalType");
		$dataObj = $hospitalTypeModel->getResource()->getByCode($this->tableCode);
		
		if($dataObj !== false){
			$hospitalTypeDTO = new Margshri_WebPortal_VO_Master_Center_Content_Type10_Hospital_HospitalTypeVO();
			/* @var $hospitalTypeVO Margshri_WebPortal_VO_Master_Center_Content_Type10_Hospital_HospitalTypeVO */
			$hospitalTypeVO  = Margshri_WebPortal_Model_DataAccess::callInstanceFunction($hospitalTypeDTO, $dataObj);
			$this->title = $hospitalTypeVO->getValue(); 
		
			$collection = Mage::getModel("webportal/Center_Content_Type10_Hospital_Hospital")->getCollection();
			$collection->getSelect()->reset()->from(array("main_table"=>$collection->getMainTable()), array("ID"=>"main_table.ID", "Value"=>"main_table.Value" , "Address"=>"main_table.Address" , "PinCode"=>"main_table.PinCode",  "LandLineNumber"=>"main_table.LandLineNumber" , "MobileNumber1"=>"main_table.MobileNumber1" , "MobileNumber2"=>"main_table.MobileNumber2" ,  "DynamicColumn1Value"=>"main_table.Category1Value", "DynamicColumn2Value"=>"main_table.Category2Value", "DynamicColumn3Value"=>"main_table.Category3Value", "main_table.HospitalTypeID"=>"", "IsPaid"=>"main_table.IsPaid",  "main_table.edit"=> new Zend_Db_Expr("'Edit'") ));
			...
			$collection->getSelect()->where('main_table.HospitalTypeID =?', $hospitalTypeVO->getID());
