<?php
class Margshri_WebPortal_Block_Frontend_Center_Content_Type2_Professional_Professional_Info extends Mage_Core_Block_Template{
	public function __construct(){
		...
		if($dataObj !== false){
			...
			*/
			...
			$collection->getSelect()->reset()->from(array("main_table"=>$collection->getMainTable()), array("ID"=>"main_table.ID", "IsPaid"=>"main_table.IsPaid", "ProfessionID"=>"main_table.ProfessionID") );
			...
			$collection->getSelect()->where("main_table.ProfessionID =?", $professionVO->getID());
