<?php
class Margshri_WebPortal_Block_Frontend_Center_Content_Type1_Bank_Branch_Info extends Mage_Core_Block_Template{
	...
	public function __construct(){
		...
		$collection->getSelect()->reset()->from(array("main_table"=>$collection->getMainTable()), array("ID"=>"main_table.ID", "Value"=>"main_table.Value" , "Code"=>"main_table.Code" , "LandLineNumber"=>"main_table.LandLineNumber" , "MICRCode"=>"main_table.MICRCode", "IFSCCode"=>"main_table.IFSCCode", "PinCode"=>"main_table.PinCode", "Address"=>"main_table.Address" ,  "MobileNumber1"=>"main_table.MobileNumber1" , "MobileNumber2"=>"main_table.MobileNumber2", "BankID"=>"main_table.BankID"  ));
		...
		$collection->getSelect()->joinLeft(array("bank"=>$collection->getTable('webportal/apctwebbank')), 'main_table.BankID = bank.ID', array("BankName"=>"bank.Value", "BankCode"=>"bank.Code", "BankWebsiteLink"=>"bank.WebsiteLink"));
