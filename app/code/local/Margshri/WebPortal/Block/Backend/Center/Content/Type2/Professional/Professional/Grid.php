<?php 
class Margshri_WebPortal_Block_Backend_Center_Content_Type2_Professional_Professional_Grid extends Mage_Adminhtml_Block_Widget_Grid{
	
	public function __construct()
	{
		parent::__construct();
		$this->setId('professional');
	...
	protected function _prepareCollection()
	{
		try{
			...
			$collection->getSelect()->joinLeft(array("professional"=>$collection->getTable("webportal/apctwebprofessional")), "e.entity_id = professional.CustomerID", array("professional.ID" =>"professional.ID", "professional.edit"=> new Zend_Db_Expr("'Edit'")) );
			$collection->getSelect()->joinLeft(array("status"=>$collection->getTable("webportal/apctstatus")), "professional.StatusID = status.ID", array("status.ID" =>"status.ID", "status.Value" =>"status.Value") );
			$collection->getSelect()->joinLeft(array("profession"=>$collection->getTable("webportal/apctwebprofession")), "professional.ProfessionID = profession.ID", array("profession.ID"=>"profession.ID", "profession.Value"=>"profession.Value") );
			...
			$collection->getSelect()->joinLeft(array("professional"=>$collection->getTable("webportal/apctwebprofessional")), "main_table.entity_id = professional.CustomerID", array("professional.ID" =>"professional.ID") );
			$collection->getSelect()->joinLeft(array("status"=>$collection->getTable("webportal/apctstatus")), "professional.StatusID = status.ID", array("status.ID" =>"status.ID", "status.Value" =>"status.Value") );
			$collection->getSelect()->joinLeft(array("profession"=>$collection->getTable("webportal/apctwebprofession")), "professional.ProfessionID = profession.ID", array("profession.ID"=>"profession.ID", "profession.Value"=>"profession.Value") );
			$collection->getSelect()->where("firstname.value =?", 'deepak');
	...
	protected function _prepareColumns()
	{
		...
		*/
		...
		$this->addColumn('created_at', array(
				...
				'width'     => '150',
		        'index'     => 'created_at',
                'renderer'  => 'webportal/Backend_Center_Content_Type2_Professional_Professional_Renderer',
		));
		
		
		
		$this->addColumn('profession.ID', array(
				'header'    =>Mage::helper('adminhtml')->__('Profession'),
				'type'  => 'options',
				'index' => 'profession.ID',
				'filter'    => false,
				'options' => Mage::getModel('webportal/Master_Center_Content_Type2_Profession_Profession')->getResource()->getOptions()
		));
		...
		/*
		$this->addColumn('main_table.entity_id', array(
		...
		$this->addColumn('firstname.value', array(
