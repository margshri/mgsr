<?php 
class Margshri_WebPortal_Block_Backend_Center_Content_Type10_Recruitment_Recruitment_Grid extends Mage_Adminhtml_Block_Widget_Grid{
	...
	protected function _prepareCollection()
	{
		try{
			...
			$collection->getSelect()->reset()->from(array("main_table"=>$collection->getMainTable()), array("main_table.ID"=>"main_table.ID", "main_table.Value"=>"main_table.Value", "main_table.DistrictID"=> "main_table.DistrictID", "main_table.edit"=> new Zend_Db_Expr("'Edit'") ));
			$collection->getSelect()->joinLeft(array("recruitmenttype"=>$collection->getTable('webportal/apctwebrecruitmenttype')), 'main_table.RecruitmentTypeID = recruitmenttype.ID', array("recruitmenttype.ID"=>"recruitmenttype.ID", "recruitmenttype.Value"=>"recruitmenttype.Value"));
	...
	protected function _prepareColumns()
	{
		...
		/*
		$this->addColumn('FirmName', array(
				'header'    =>Mage::helper('adminhtml')->__('Firm Name'),
				'index'     =>'directory.FirmName'
		));
		
		$this->addColumn('ContactPersonName', array(
				'header'    =>Mage::helper('adminhtml')->__('Contact Person Name'),
				'index'     =>'directory.ContactPersonName',
		));

		*/
		
		$this->addColumn('RecruitmentTypeID', array(
				'header'    =>Mage::helper('adminhtml')->__('Recruitment Type'),
				'type'  => 'options',
				'index' => 'recruitmenttype.ID',
				'options' => Mage::getModel('webportal/Master_Center_Content_Type10_Recruitment_RecruitmentType')->getResource()->getOptions()
		));
		...
		$this->addColumn('Edit', array(
				...
				'renderer'  => 'webportal/Backend_Center_Content_Type10_Recruitment_Recruitment_Renderer',
