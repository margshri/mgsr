<?php 
class Margshri_WebPortal_Block_Backend_Center_Content_Type2_BloodDonor_BloodDonor_Grid extends Mage_Adminhtml_Block_Widget_Grid{
	
	public function __construct()
	{
		parent::__construct();
		$this->setId('blooddonor');
	...
	protected function _prepareCollection()
	{
		try{
			...
			->addAttributeToSelect('bloodgroup')
			...
			$collection->getSelect()->joinLeft(array("blooddonor"=>$collection->getTable("webportal/apctwebblooddonor")), "e.entity_id = blooddonor.CustomerID", array("blooddonor.ID" =>"blooddonor.ID", "blooddonor.IsDonated" =>"blooddonor.IsDonated", "blooddonor.BloodGroupID" =>"blooddonor.BloodGroupID", "blooddonor.LastDonatedAt" =>"blooddonor.LastDonatedAt", "blooddonor.edit"=> new Zend_Db_Expr("'Edit'")) );
			//$collection->getSelect()->joinLeft(array("bloodgroup"=>$collection->getTable("webportal/apctwebbloodgroup")), "e.entity_id = bloodgroup.ID", array("bloodgroup.value" =>"blooddonor.ID", "blooddonor.edit"=> new Zend_Db_Expr("'Edit'")) );
			$collection->getSelect()->joinLeft(array("status"=>$collection->getTable("webportal/apctstatus")), "blooddonor.StatusID = status.ID", array("status.ID" =>"status.ID", "status.Value" =>"status.Value") );
	...
	protected function _prepareColumns()
	{
		...
		$this->addColumn('bloodgroup', array(
				'header'    => Mage::helper('customer')->__('Blood Group'),
				'width'     => '150',
				'index'     => 'bloodgroup'
		));
		...
		/*
		$this->addColumn('BloodGroupID', array(
				'header'    =>Mage::helper('adminhtml')->__('Blood Group'),
				'type'  => 'options',
				'filter'    => false,
				'index' => 'blooddonor.BloodGroupID',
				'options' => Mage::getModel('webportal/Master_Center_Content_Type2_BloodDonor_BloodGroup')->getResource()->getActiveOptions()
		));
		*/
		...
		$this->addColumn('gender', array(
				...
				'index'     => 'gender',
				'type'  => 'options',
				// 'renderer'  => 'webportal/Backend_Center_Content_Type2_BloodDonor_BloodDonor_Renderer'
		...
		$isDonatedOptions = array(0=>"No", 1=>"Yes");
		$this->addColumn('IsDonated', array(
				'header'    =>Mage::helper('adminhtml')->__('IsDonated'),
				'type'  => 'options',
				'index' => 'blooddonor.IsDonated',
				'filter'    => false,
				'options' => $isDonatedOptions
		));
		
		$this->addColumn('LastDonatedAt', array(
				'header'    => Mage::helper('customer')->__('LastDonatedAt'),
				'width'     => '150',
				'index'     => 'blooddonor.LastDonatedAt'
		));
		
		$this->addColumn('Edit', array(
				'header'    =>Mage::helper('adminhtml')->__('Edit'),
				'index'     =>'blooddonor.edit',
				'align'     => 'left',
				'width'     => '50px',
				'sortable'  => false,
				'filter'    => false,
