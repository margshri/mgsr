<?php 
class Margshri_Transport_Backend_Master_Vahicale_OwnerController extends Mage_Adminhtml_Controller_Action{
	
	private $gridBlock ='transport/Backend_Master_Vahicale_Owner_Grid';
	private $formBlock ='transport/Backend_Master_Vahicale_Owner_Form';
	
	
	protected function _init(){
	    $ownerID = $this->getRequest()->getParam('ID');
	    ...
	    if($ownerID != null){
	        ...
	        $ownerDataObj = $ownerModel->getResource()->getByID($ownerID);
	        if($ownerDataObj !== false){
	            $ownerDTO = new Margshri_Transport_VO_Master_Vahicale_OwnerVO();
	            $ownerVO = Margshri_Transport_Helper_Data::callInstanceFunction($ownerDTO, $ownerDataObj);
			}
		}
		Mage::register('CurrentOwnerVO', $ownerVO);
		return $ownerVO;
	}
	...
	public function editAction(){
		...
		$this->loadLayout();
		$this->_addContent(
		    $this->getLayout()->createBlock($this->formBlock)->setVahicaleID($ownerVO->getID())
		);
		$this->renderLayout();
	}
	...
	public function saveAction(){
    	...
    	try {
    		...
    		$ownerDataObj = json_decode($post["OwnerDataObj"],true);
    		if($ownerDataObj == null){
    		    Mage::throwException("Invalid Form Data.");
    		}
    		$ownerDTO = new Margshri_Transport_VO_Master_Vahicale_OwnerVO();
    		/* @var $ownerVO Margshri_Transport_VO_Master_Vahicale_OwnerVO */
